package tohope.app.yuedan.util.cashinpututil;
...
/**
 * 逸动无线
 * Created by ChenZhihong on 2017/5/17.
 * 仿微信转账输入框规则（可能不全）
 * 1 小数点后两位
 * 2 起始位置输入小数点，显示0.
 * 3 删除到第一位置是小数点的时候，第一位置为0，避免出现小数点在第一位置的情况
 * 修复：
 1、7.0小米4手机对于输入的 .  和程序中的“.”无法 equal相等，估改为正则表达式匹配
 *  参考：  InputFilter[] is = {new CashierInputFilter()};
 *           etInputMoney.setFilters(is);
 */
